...
switch ($cmd) {
    case 'download':
        ...
        if (($row['page_download']) && ($row['page_download'] != ',,')) {
            if (!strpos($row['page_download'], strval($current_user_level))) {
                just_die($lang['msg']['no_level']);
            }
        }
        if (empty($row)) {
    ...
    case 'pid':
        ...
        if (!$content || $current_admin_level) {
            // get cat info
        ...
        if ($current_admin_level) {
            $txt['main_body'] = sprintf($lang['edit_in_acp'], $page_id).rte_inline('.editable').$txt['main_body'];
        }
