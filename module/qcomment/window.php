...
switch ($mode) {
    case 'latest':
        ...
        $output = qcache_get('qcomment_window_lastest_'.$mod_id);
        if (!$output) {
            // load mod's template
            $tpl = load_tpl('mod', 'module_qcomment_window.tpl');

            $output = $txt['block_list'] = '';
            $res = sql_query("SELECT item_title, item_url, comment_title, comment_date, comment_user FROM ".$db_prefix."qcomment WHERE mod_id = '$mod_id' ORDER BY comment_id DESC LIMIT $items");
            while ($row = sql_fetch_array($res)) {
                $foo = clean_get_query('permalink_request', true, $row['item_url']);
                if (substr($foo, -1, 1) == '?') {
                    $foo = substr($foo, 0, -1);
                }
                $row['item_url'] = $foo;
                $row['comment_date'] = convert_date($row['comment_date'], 'med');
                $txt['block_list'] .= quick_tpl($tpl_block['list'], $row);
            }
            $output = quick_tpl($tpl, $txt);
            qcache_update('qcomment_window_lastest_'.$mod_id, $output);
        }
    break;
    ...
    case 'most':
        ...
        $output = qcache_get('qcomment_window_most_'.$mod_id);
        if (!$output) {
            // load mod's template
            $tpl = load_tpl('mod', 'module_qcomment_window.tpl');

            $output = $txt['block_list'] = '';
            $res = sql_query("SELECT COUNT(*) AS total, item_title, item_url, comment_title FROM ".$db_prefix."qcomment WHERE mod_id='$mod_id' GROUP BY (item_id) ORDER BY total DESC LIMIT $items");
            while ($row = sql_fetch_array($res)) {
                $foo = clean_get_query('permalink_request', true, $row['item_url']);
                if (substr($foo, -1, 1) == '?') {
                    $foo = substr($foo, 0, -1);
            ...
            $output = quick_tpl($tpl, $txt);
            qcache_update('qcomment_window_most_'.$mod_id, $output);
        }
    break;
    ...
    case 'invalid':
        $output = '<!-- qcomment error: invalid mod_id, please define it in acp! -->';
    break;
