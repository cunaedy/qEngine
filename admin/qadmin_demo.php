...
// $qadmin_def['title']['allow_ajax'] = true;			// see ajax support below
...
// $qadmin_def['rte']['allow_ajax'] = true;				// see ajax support below
...
/* ajax supports
by enabling ajax supports, you can create a script to be proceed by ajax request.
This allow qadmin to process the request to: update, insert (new), delete file, delete item, etc
The user interface is not covered, you must create your own UI

- After setting the $qadmin_cfg['allow_ajax'] value, you must also inform qadmin on which fields support ajax (see news_title & news_body above)
- Some external scripts with multiple forms may need to add suffix to the form field name (eg: news_title >> to >> news_title_9), define the format in $qadmin_cfg['allow_ajax_suffix']
  qAdmin will then remove any suffix from field name before processing. Resulting name: [form_field_name][separator, as defined in cfg][primary_val], eg: news_title_9, news_title_10
*/
# $qadmin_cfg['allow_ajax'] = true;							// set to true to allow ajax access to qadmin
# $qadmin_cfg['allow_ajax_suffix'] = '_';
