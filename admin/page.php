...
function post_func($cmd, $id, $savenew = false, $old_values, $new_values, $isAjax)
{
...
$qadmin_def['page_title']['allow_ajax'] = true;
if ($mode == 'new') {
...
$qadmin_def['page_body']['allow_ajax'] = true;
if ($id) {
...
if ($rule_def['page_attachment']) {
    ...
    // page_download :: string :: 1
    $qadmin_def['page_download']['title'] = 'Who can download attachment?';
    $qadmin_def['page_download']['field'] = 'page_download';
    $qadmin_def['page_download']['type'] = 'multi,';
    $qadmin_def['page_download']['option'] = array(0 => $lang['l_guest']) + $user_level_def;
    $qadmin_def['page_download']['value'] = $id ? 'sql' : $rule_def['page_download'];
}
...
$qadmin_def['page_list']['value'] = $id ? 'sql' : ($rule_def['cat_list'] ? 1 : 0);
...
if ($id) {
    ...
    $qadmin_def['notes1']['value'] = 'You can overwrite this content &amp; design by creating a <kbd>/skins/[your_skin]/page_'.$id.'.tpl</kbd> file.';
}
